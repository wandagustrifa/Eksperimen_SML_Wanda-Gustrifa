# C:\gitwork\Eksperimen_SML_Wanda-Gustrifa\Membangun_model\modelling_tuning.py
...
def tune_and_train_model(input_filepath):
    ...
    grid_search = GridSearchCV(
        estimator=lr, param_grid=param_grid, cv=5, scoring='roc_auc', n_jobs=-1, verbose=2
    )
    ...
    # NAMA FOLDER ARTEFAK UTAMA UNTUK MODEL
    model_folder_name = "model" 

    with mlflow.start_run(run_name="LogisticRegression_Tuned_Model"):
        # Log parameter terbaik
        for param, value in grid_search.best_params_.items():
            mlflow.log_param(f"best_{param}", value)
        mlflow.log_param("tuning_scoring_metric", 'roc_auc')
        ...
        # Log metrik secara manual (sesuai kriteria Skilled + Advance)
        ...
        # Metrik tambahan dari confusion matrix (untuk kriteria Advance)
        tn, fp, fn, tp = confusion_matrix(y_test, y_pred).ravel()
        specificity = tn / (tn + fp) if (tn + fp) != 0 else 0
        false_positive_rate = fp / (fp + tn) if (fp + tn) != 0 else 0
        mlflow.log_metric("specificity", specificity) # Metrik tambahan 2
        mlflow.log_metric("false_positive_rate", false_positive_rate) # Metrik tambahan 3
        ...
        mlflow.sklearn.log_model(best_model, model_folder_name) # Nama folder artefak model
